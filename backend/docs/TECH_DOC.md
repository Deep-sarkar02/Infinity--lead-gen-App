...
| **Hosting** | Docker (frontend + backend) on VM/cloud; AWS Lambda for webhooks at scale |
| **Secrets** | Root `.env` on server; AWS Secrets Manager for Lambda |
...
Git push → GitHub Actions
  ...
  ├── unit tests
  ├── build
  └── deploy (Docker on VM / CI pipeline)
```
