...
const statConfig: Record<
  StatVariant,
  { bg: string; accent: string; labelColor: string; icon: string; label: string }
> = {
  verified: {
    bg: "bg-il-success-95",
...
function ProfileStatCard({ variant, value }: { variant: StatVariant; value: number }) {
  ...
  return (
    ...
    >
      <span
        className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-white text-lg font-bold shadow-card ${config.accent}`}
      >
        {config.icon}
      </span>
